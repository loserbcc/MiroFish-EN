...
Reads nodes from the Zep graph, filtering for nodes matching predefined entity types
...
class ZepEntityReader:
    ...
    2. Filter for nodes matching predefined entity types (nodes with labels beyond just Entity)
    3. Get related edges and associated node information for each entity
    """

    def __init__(self, api_key: Optional[str] = None):
        """Initialize the entity reader service (using the adapter factory)"""
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all nodes in the graph (with retry mechanism)
        ...
        # Use retry mechanism to call adapter API
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all edges in the graph (with retry mechanism)
    ...
    def filter_defined_entities(
        ...
        - If a node's Labels contain tags beyond "Entity" and "Node", it matches a predefined type, keep it
        ...
        # Fallback: if no nodes with business labels are found, return all Entity nodes
        ...
        if not filtered_entities and total_count > 0 and not defined_entity_types:
            logger.warning(
                ...
                "This may be because the Graphiti backend is being used without ontology configuration."
            )
            for node in all_nodes:
                entity = EntityNode(
                    uuid=node["uuid"],
