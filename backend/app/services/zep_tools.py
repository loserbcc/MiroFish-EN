...
class InsightForgeResult:
    ...
    Contains retrieval results from multiple sub-queries, along with consolidated analysis
    """
    query: str
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        # Interview content from each agent
        if self.interviews:
...
class ZepToolsService:
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        """Initialize Zep tools service (using singleton)"""
        # Use singleton to get the adapter (avoid repeated initialization)
    ...
    def search_graph(
        ...
        try:
            ...
            # Parse edge search results (adapter returns a list of GraphEdge objects)
            ...
            # Parse node search results (adapter returns a list of GraphNode objects)
    ...
    def _local_search(
        ...
        Gets all edges/nodes and then performs local keyword matching
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        nodes = self._call_with_retry(
            ...
            operation_name=f"get_nodes(graph={graph_id})"
        )
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        edges = self._call_with_retry(
            ...
            operation_name=f"get_edges(graph={graph_id})"
        )
        ...
        for edge in edges:
            ...
            # Add temporal information (adapter already has these fields)
            if include_temporal:
...
Return the sub-question list in JSON format."""
    ...
    def panorama_search(
        ...
        This tool is suited for scenarios requiring a full picture of events and tracking evolution processes.
    ...
    def interview_agents(
        ...
        # Add optimized prefix to prevent agents from calling tools and reply with text directly
        ...
        # Step 4: Call real interview API (no platform specified, defaults to dual-platform simultaneous interview)
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Merge responses from both platforms
                response_parts = []
                if twitter_response:
                    response_parts.append(f"[Twitter Platform Response]\n{twitter_response}")
                if reddit_response:
                    response_parts.append(f"[Reddit Platform Response]\n{reddit_response}")
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable interview candidates from the simulated agent list based on interview requirements.
