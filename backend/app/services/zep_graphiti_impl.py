...
Replaces Zep Cloud, implementing the ZepClientAdapter interface.

MVP scope:
- Graph creation/deletion (isolation via group_id)
- Episode addition (single/batch)
- Node/edge retrieval
- Semantic search

Ontology is a no-op in the MVP stage.
"""
...
def _ensure_async_loop():
    """Ensure the background event loop has been started"""
...
def _run_async(coro):
    ...
    This ensures the Neo4j driver is always bound to the same loop, avoiding cross-loop issues.
    """
    _ensure_async_loop()
...
class DashScopeEmbedderWrapper:
    ...
    DashScope API has batch size limits (max 10); graphiti-core's OpenAIEmbedder
    sends all inputs at once. This wrapper chunks the requests.
    ...
    def __init__(self, embedder: Any, max_batch_size: int = 10):
        ...
        # Copy the original embedder's attributes for compatibility
        if hasattr(embedder, 'config'):
            self.config = embedder.config

    async def create(self, input_data) -> list[float]:
        """Single embedding request (direct passthrough)"""
...
def _create_dashscope_embedder_wrapper(base_embedder: Any, max_batch_size: int = 10) -> Any:
    ...
    try:
        ...
        # Fallback: return a plain wrapper
...
class GraphitiClient(ZepClientAdapter):
    ...
    Uses the graphiti-core library to connect to a Neo4j graph database.
    Multi-graph isolation is achieved via the group_id parameter (corresponding to MiroFish's graph_id).
    """
    ...
    def __init__(
        ...
        # Lazy initialization of the Graphiti instance
        self._graphiti = None
        self._driver = None
        ...
        # Store ontology definitions (MVP stage: only recorded, not enforced)
        self._ontology_cache: Dict[str, Dict[str, Any]] = {}

    def _ensure_initialized(self):
        """Ensure Graphiti has been initialized"""
        ...
        try:
            ...
            # Apply Neo4j attribute sanitization patch (Issue #683 workaround)
            ...
            logger.info("Graphiti client initialization complete")
    ...
    def _build_default_llm_client(self) -> Any:
        """
        Build the default Graphiti LLM client (OpenAI-compatible)

        Graphiti defaults to `gpt-4.1-mini`, which is usually not suitable for OpenAI-compatible
        services like DashScope; here we prioritize:
        - GRAPHITI_LLM_MODEL (if available)
        - Otherwise use LLM_MODEL_NAME (consistent with MiroFish existing config)
        """
    ...
    def _build_default_embedder(self) -> Any:
        """
        Build the default Graphiti Embedder (OpenAI-compatible /embeddings)

        The default embedding model is `text-embedding-3-small` (OpenAI); for DashScope,
        explicit configuration is needed:
        - GRAPHITI_EMBEDDING_MODEL=text-embedding-v4

        Note: DashScope API has batch size limits (max 10), handled by DashScopeEmbedderWrapper.
        """
        ...
        # DashScope API has batch size limits, needs wrapping
    ...
    def create_graph(self, graph_id: str, name: str, description: str) -> None:
        ...
        Graphiti has no explicit graph creation API; data is automatically isolated via group_id.
        Only metadata is recorded here; actual data is created during add_episode.
        """
    ...
    def delete_graph(self, graph_id: str) -> None:
        """
        Delete a graph (delete all data related to the group_id)

        Uses Cypher to directly delete all nodes and edges matching the group_id in Neo4j.
        All Graphiti nodes (Entity, Episodic, etc.) have a group_id property,
        so a single generic query covers everything.
        """
        self._ensure_initialized()

        async def _delete():
            # Delete all nodes with this group_id (cascading edge deletion)
            # Both Entity and Episodic nodes in Graphiti have group_id, no need to delete separately
    ...
    def set_ontology(
        ...
        MVP note: Graphiti does not support the exact same ontology API as Zep Cloud.
        Only definitions are cached here, which can be used for:
        1. Prompt hints when adding episodes
        2. Type mapping during future alignment

        Full parity stage can implement:
        - Dynamically generate Pydantic Entity/Edge models to pass to add_episode
        - Create type constraints in Neo4j
        """
    ...
    def get_episode_status(self, episode_uuid: str) -> EpisodeStatus:
        ...
        Graphiti processes episodes synchronously; once added, it's already processed.
        """
    ...
    def wait_for_episode(self, episode_uuid: str, timeout: int = 300) -> bool:
        ...
        Graphiti processes synchronously, returns True directly.
        """
        return True
    ...
    def get_all_nodes(self, graph_id: str) -> List[GraphNode]:
        ...
        for record in records:
            ...
            # Filter out attributes that were already extracted individually
            attributes = {
    ...
    def get_node(self, node_uuid: str) -> Optional[GraphNode]:
        ...
        async def _get_node():
            # Find node by uuid, no label restriction (more flexible)
            records, _, _ = await self._driver.execute_query(
                """
                MATCH (n {uuid: $uuid})
                RETURN
        ...
        if not records:
            logger.debug(f"get_node: No node found with uuid={node_uuid}")
            return None
    ...
    def get_node_edges(self, node_uuid: str) -> List[GraphEdge]:
        ...
        async def _get_edges():
            # No label restriction on nodes, match by uuid, get bidirectional edges
        ...
        if not records:
            logger.debug(f"get_node_edges: Node uuid={node_uuid} has no associated edges")
        return [self._record_to_edge(record) for record in records]

    # ==================== Edge operations ====================

    def get_all_edges(self, graph_id: str) -> List[GraphEdge]:
        """Get all edges in the graph (filtered via node group_id)"""
        self._ensure_initialized()

        async def _get_edges():
            # Filter edges via node group_id, use DISTINCT to avoid duplicates
            ...
            logger.warning(
                ...
                f"Possible reasons: 1) Graph has no edges 2) Graphiti schema mismatch"
            )
            return []
    ...
    def _is_openai_compatible_only(self) -> bool:
        """
        Detect whether a non-standard OpenAI API is being used (e.g., DashScope, Azure, etc.)

        These APIs may not support the logprobs feature required by cross_encoder,
        requiring fallback to RRF reranking.

        Can be overridden via GRAPHITI_FORCE_CROSS_ENCODER=true to force cross_encoder usage
        (for compatible services confirmed to support logprobs).
        """
        import os

        # Explicit override: force cross_encoder usage
        ...
        if not base_url or 'api.openai.com' in base_url:
            return False
        # Non-standard API (DashScope, Azure, local deployment, etc.)
    ...
    def search(
        ...
        limit: int = 10,
        scope: str = "edges",
        reranker: str = "rrf"  # Default changed to rrf, safer
    ) -> SearchResult:
        """
        Graph hybrid search
        ...
        Note: reranker="cross_encoder" requires the OpenAI API to support logprobs;
        non-standard APIs (e.g., DashScope) will automatically downgrade to rrf.
        """
        self._ensure_initialized()

        # Non-standard OpenAI APIs don't support cross_encoder, force rrf
        if reranker == "cross_encoder" and self._is_openai_compatible_only():
            logger.info("Detected non-standard OpenAI API, downgrading cross_encoder to rrf")
            reranker = "rrf"
        ...
        async def _do_search():
            ...
            # Check if the search_() method exists (public advanced search API)
            has_search_method = hasattr(self._graphiti, 'search_')

            if not has_search_method:
                # Fallback: use the simple search() API
                ...
                try:
                    ...
                    # Simple search mainly returns edges
                    if results:
    ...
    def _record_to_edge(self, record: Dict[str, Any]) -> GraphEdge:
        """Convert a Neo4j query result to GraphEdge"""
        props = record.get("props", {})
        attributes = {
            k: v for k, v in props.items()
