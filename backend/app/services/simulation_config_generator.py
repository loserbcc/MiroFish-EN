...
document content, and knowledge graph information. Fully automated, no manual parameter setting needed.
...
class SimulationConfigGenerator:
    ...
    Uses LLM to analyze simulation requirements, document content, and knowledge graph entity information,
...
- The user base is Chinese, must conform to Beijing time daily schedule habits
...
- General pattern: low activity in early morning, gradual increase in morning, moderate during work hours, evening peak
- **Important**: The example values below are for reference only. You need to adjust specific time periods based on event nature and participant group characteristics
  ...
  - Example: Breaking news may lead to late-night discussions, off_peak_hours can be shortened accordingly
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time config results and validate agents_per_hour values don't exceed total agent count"""
        # Get raw values
...
- Describe the public opinion narrative direction
...
{{
    ...
    "narrative_direction": "<public opinion narrative direction description>",
    "initial_posts": [
...
- **Schedule must conform to Chinese daily habits**: almost no activity from 0-5 AM, most active from 7-10 PM
