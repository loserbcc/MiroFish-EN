...
- We need to simulate reactions from various parties in public opinion events and information propagation paths
...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual person. When a person does not fit any other more specific person type, classify them here.
   - `Organization`: Fallback type for any organization. When an organization does not fit any other more specific organization type, classify them here.
...
- Various people will appear in the text, such as "elementary school teacher", "random passerby", "some netizen"
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50,000 characters, truncate (only affects content sent to LLM, not knowledge graph construction)
...
4. All entity types must be real-world agents that can express opinions, not abstract concepts
