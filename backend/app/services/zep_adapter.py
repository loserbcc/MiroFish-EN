...
Defines a unified Zep client abstract interface, supporting cloud/graphiti dual-implementation switching.
MVP stage goal: No dependency on Zep Cloud, run through the "build graph -> read entities -> search -> report" core pipeline.
"""
...
class ZepClientAdapter(ABC):
    ...
    - wait_for_episode: Wait for episode processing to complete (Graphiti processes synchronously, returns immediately)
    """
    ...
    def set_ontology(
        ...
        MVP note: Graphiti implementation can no-op or only log for prompt hints.
    ...
    def add_episode_batch(
        ...
        """
        Add episodes to the graph in batch

        Args:
            graph_id: Graph ID
            episodes: Episode list, each item contains {"data": str, "type": str}
    ...
    def get_episode_status(self, episode_uuid: str) -> EpisodeStatus:
        ...
        Returns:
            EpisodeStatus containing uuid and processed status
        """
        ...
    ...
    def wait_for_episode(self, episode_uuid: str, timeout: int = 300) -> bool:
        ...
        Default implementation: polls get_episode_status until processed=True or timeout.
    ...
    def search(
        ...
        Returns:
            SearchResult containing matched nodes and edges
        """
        ...
