...
Runs simulations in the background, records each Agent's actions, supports real-time status monitoring
"""
...
# Flag for whether cleanup function is registered
...
def _get_simulation_python() -> str:
    """
    Get the Python interpreter path for the simulation environment

    Priority:
    1. SIMULATION_PYTHON environment variable
    2. .venv-simulation/bin/python under the project directory
    3. Fall back to current Python interpreter

    The simulation environment requires camel-oasis/camel-ai (neo4j==5.23.0),
    which is incompatible with the main environment's graphiti-core (neo4j>=5.26.0) and needs isolated execution.
    """
    # 1. Environment variable takes priority
    env_python = os.environ.get('SIMULATION_PYTHON')
    if env_python and os.path.isfile(env_python):
        logger.info(f"Using simulation Python from environment variable: {env_python}")
        return env_python

    # 2. Check for isolated simulation environment under project directory
    backend_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    sim_venv_python = os.path.join(backend_dir, '.venv-simulation', 'bin', 'python')
    if os.path.isfile(sim_venv_python):
        logger.info(f"Using isolated simulation environment: {sim_venv_python}")
        return sim_venv_python

    # 3. Fall back to current interpreter (may fail due to dependency conflicts)
    logger.warning("Isolated simulation environment not found, using current Python (possible dependency conflicts)")
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            cmd = [
                _get_simulation_python(),  # Use isolated simulation environment
                script_path,
            ...
            # Create main log file to avoid process blocking due to full stdout/stderr pipe buffers
    ...
    def register_cleanup(cls):
        ...
        # If not debug mode, this env var doesn't exist, still need to register
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview on both platforms in dual-platform mode, return combined results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": Default to only interview on Twitter
                - "reddit": Default to only interview on Reddit
