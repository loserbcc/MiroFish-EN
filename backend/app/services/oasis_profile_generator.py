...
class OasisProfileGenerator:
    ...
    1. Calls Zep knowledge graph retrieval to obtain richer context
    ...
    def __init__(
        ...
        # Zep client for retrieving rich context (using adapter factory)
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep knowledge graph hybrid search to obtain rich information about an entity

        Zep does not have a built-in hybrid search interface, so edges and nodes must be searched
        separately and results merged. Uses parallel requests for efficiency.
    ...
    def _generate_profile_with_llm(
        ...
        # Try multiple times until successful or max retries reached
        max_attempts = 3
        last_error = None
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace actual newline characters within strings with spaces
                s = s.replace('\n', ' ').replace('\r', ' ')
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, maximizing fidelity to known real-world circumstances. You must return valid JSON format. All string values must not contain unescaped newline characters. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, maximizing fidelity to known real-world circumstances.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Stances and viewpoints (attitude toward topics, content likely to provoke or move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (an important part of the persona: introduce this individual's connection to the event, and their existing actions and reactions within the event)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, maximizing fidelity to known real-world circumstances.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   - Institutional basic information (official name, nature of the institution, founding background, primary functions)
   ...
   - Institutional memory (an important part of the institutional persona: introduce this institution's connection to the event, and its existing actions and reactions within the event)
...
- Use English (gender field must be the English word "other")
...
- Institutional account communications must match its identity and positioning"""
    ...
    def set_graph_id(self, graph_id: str):
        """Set knowledge graph ID for Zep retrieval"""
        self.graph_id = graph_id
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Knowledge graph ID, used for Zep retrieval to obtain richer context
