...
# ============== API 2: Build knowledge graph ==============

@graph_bp.route('/build', methods=['POST'])
def build_graph():
    """
    API 2: Build knowledge graph by project_id
    ...
    try:
        ...
        # Check configuration (only cloud mode requires ZEP_API_KEY)
        if Config.ZEP_BACKEND == 'cloud' and not Config.ZEP_API_KEY:
            logger.error("Configuration error: ZEP_API_KEY not configured")
            return jsonify({
                "success": False,
