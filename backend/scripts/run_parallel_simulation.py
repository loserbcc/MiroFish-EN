...
- Does not close environment immediately after simulation, enters command-waiting mode
...
# This fixes issues with OASIS third-party libraries not specifying encoding when reading files
# ============================================================
import sys
import os
...
# Add filter immediately on module load, ensuring it takes effect before camel code executes
...
class ParallelIPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items that need interviews on both platforms
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from the database and supplement with complete context information
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (preserve full content, don't truncate)
            simplified_args = {}
            ...
            # Supplement context info (post content, username, etc.)
...
def _enrich_action_context(
    ...
    """
    Supplement context info for actions (post content, username, etc.)
    ...
    try:
        ...
        # Context supplementation failure does not affect main flow
...
async def main():
    ...
    # Create shutdown event at the start of main, ensuring the entire program can respond to exit signals
