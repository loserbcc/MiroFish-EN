...
const actionIds = ref(new Set()) // Set for dedup
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.runner_status === 'failed') {
        ...
        phase.value = 2  // Enter completed phase (allow viewing logs/retry)
        stopPolling()
        emit('update-status', 'error')
        return
      }
      ...
      // Extra check: if backend has not updated runner_status but platforms report completion
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine if platform was enabled via actions_count (count > 0 or was running)
