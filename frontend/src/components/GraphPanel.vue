...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Ensures all edges distribute under same reference frame without overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from right side, loop back
    ...
    // Calculate curve control points - dynamic by edge count and distance
  ...
  // Link labels background (white bg for readability)
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (no rotation, horizontal)
