            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section/Subsection Content Generated -->
                  ...
                  <!-- Section Complete (all subsections done) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### (?:分析的子问题|Sub-Questions)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // Extract relation chains - full extraction
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - full extraction
    ...
    // Extract historical/expired facts - full extraction
    ...
    // Extract involved entities - full extraction
  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // If only one platform answer, use as primary
        // So content is available regardless of platform displayed
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, set as twitterAnswer for default display
          interview.twitterAnswer = interview.redditAnswer
        } else if (twitterMatch && !redditMatch) {
          // Only Twitter answer, also set as redditAnswer
          interview.redditAnswer = interview.twitterAnswer
        } else if (!twitterMatch && !redditMatch) {
          // If no clear platform split, use as whole answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - full extraction
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // More robust splitting: find all number+dot positions
      // Supported formats:
      // - "1.  \ncontent" (number+dot+space+newline+content)
      // - "\n\n2.  \ncontent" (newline+number+dot+space+newline+content)
      // Use lenient matching: number+dot+space after start or newline
      const numberPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      const matches = []
      let match
      ...
      // If no numbers found or only one, return whole
      if (matches.length <= 1) {
        // Try to remove leading number (format: 1.  \n or 1. )
      ...
      // If split successful and count is reasonable, return split result
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 (## xxx) since section title is displayed in outer layer
  ...
  // Process newlines - empty lines become paragraphs, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
...
const getLogLevelClass = (log) => {
  ...
  // INFO Use default color, do not mark as success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_content / subsection_content - content generated (but full section may not be done)
          // Don't update generatedSections here, just track progress
          if (log.action === 'section_content' || log.action === 'subsection_content') {
            // When subsection content is generated, keep main section loading state
            // Because complete content is provided at section_complete
          }
          
          // section_complete - full section (with all subsections) generation complete
          // details.content contains merged complete content
          // Note: only update content on main section complete, ignore subsection complete
          if (log.action === 'section_complete') {
            const mainIndex = getMainSectionIndex(log.section_index)
            // Only update content and clear loading for main section (section_index < 100)
            ...
            // Don't clear currentSectionIndex on subsection complete, keep showing loading
          }
...
const extractFinalContent = (response) => {
  ...
  // Try to extract content within <final_answer> tag
  ...
  const altFinalMatch = response.match(/(?:Final Answer|最终答案)[:：]\s*\n*([\s\S]*)$/i)
  if (altFinalMatch) {
  ...
  // If content is long with markdown, try removing thought process
